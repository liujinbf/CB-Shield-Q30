...
默认会依次尝试:
  1. 与安装脚本同目录的 cbshield-kwrt-runtime.tgz
...
require_root() {
    [ "$(id -u)" = "0" ] || fatal "请使用 root 执行安装脚本"
}

ensure_base_tools() {
    for cmd in tar uci opkg ubus; do
        command -v "$cmd" >/dev/null 2>&1 || fatal "缺少基础命令: $cmd"
    done
}
...
show_summary() {
    ...
    log "安装完成，检测板型: $board_name"
    log "原有文件备份目录: $BACKUP_DIR"
    log "网络与 LuCI 正在后台重载，预计 5-10 秒内完成"
    log "默认管理地址将切换为: http://192.168.10.1/"
    log "默认无线名称应为: CB-Shield-Office / CB-Shield-Office-5G"
    log "如需查看后台重载日志: $RESTART_LOG"
}
...
main() {
    ...
    BUNDLE_PATH="$(resolve_bundle "${1:-}")" || fatal "未找到 runtime bundle，请把 cbshield-kwrt-runtime.tgz 上传到 /tmp 或与安装脚本放在同一目录"
    [ -f "$BUNDLE_PATH" ] || fatal "bundle 不存在: $BUNDLE_PATH"

    log "准备在已运行的 Kwrt 系统上安装 CB-Shield 叠加层"
    install_missing_packages
